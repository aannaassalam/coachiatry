import { addStatusByCoach } from "@/external-api/functions/status.api";
...
function AddStatusModal({
  ...
  const { mutate, isPending } = useMutation({
    ...
    meta: {
      invalidateQueries: ["status", userId]
    }
  });
      ...
      >
        <div className="w-full p-4 flex flex-col items-start gap-3">
          ...
          <Button
            onClick={() => {
              if (title.trim()) {
                mutate({ title, color: selectedColor, user: userId as string });
              }
            }}
            isLoading={isPending}
            disabled={!title.trim()}
